#!/usr/bin/env bash
# Runs the recipe-creator demo end to end: starts a local OTLP receiver,
# drives two real Google ADK sessions (one with real sub-agent
# delegation, one single-agent), converts and analyzes what got
# captured, and prints the report plus where to find the full HTML
# version.
#
...
if [[ -z "${GOOGLE_API_KEY:-}${GEMINI_API_KEY:-}" ]]; then
  echo "Neither GOOGLE_API_KEY nor GEMINI_API_KEY is set. Export one and re-run." >&2
  exit 1
fi
export GOOGLE_API_KEY="${GOOGLE_API_KEY:-${GEMINI_API_KEY:-}}"

if [[ ! -d "$HERE/venv" ]]; then
  echo "First run: setting up a local venv with Google ADK..."
...
WORK_DIR="$(mktemp -d /tmp/redundo-demo-google-adk-recipe.XXXXXX)"
...
echo "Running the two Google ADK sessions (this takes a minute or two)..."
"$HERE/venv/bin/python3" "$HERE/recipe_bot.py"
