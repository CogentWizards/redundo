...
**likely legitimate**, or **unclassified**, plus a fourth, lower-confidence
**near-duplicate** bucket for similar-but-not-identical repeats. The
schema in between is the real contract, not the pipe — if your framework
isn't supported yet, skip
`adapt` and pipe in your own NDJSON matching
[the schema](docs/schema.md); `analyze` doesn't know or care where its
input came from.
...
loaded corpus ever sets that key, the line is omitted entirely rather than
reporting a fabricated "0%" — silence here means "this dimension can't be
spoken to for this data," not "everything failed."
...
Five specific edge cases in the schema contract itself — what happens
with no `parent_id`, how branching and parallel tasks are handled, how a
chain of repeats gets paired — are resolved explicitly, not left
ambiguous. See [docs/schema.md](docs/schema.md) for exactly what each one
decides and why.
